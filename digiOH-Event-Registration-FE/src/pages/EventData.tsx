...
import { ChevronUpIcon, MagnifyingGlassIcon, ChevronDownIcon, EnvelopeIcon, InboxIcon, ArrowDownOnSquareIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const EventData = () => {
    ...
    // Fetching guests
    const loadGuests = (eventId: number) => {
      if (!token) {
        return;
      }

      const queryString = getQueryParams();
      getGuestsSearchFilter(eventId.toString(), queryString)
        .then(response => {
          setFilteredGuests(response.guests);
          setCurrentPage(response.pagination.page);
          setTotalPages(response.pagination.totalPages);
    ...
    const handleEventSelect = (event: Event) => {
      ...
      if (token) {
        console.log("a")
        loadConfirmationCounts(event.id)
        loadGuests(event.id);
      }
    };
    ...
    const handleExportToExcel = async () => {
      if (selectedEvent) {
        try {
          ...
          link.href = url;
          link.setAttribute('download', `${selectedEvent.name} Guests Data.xlsx`);
    ...
    const updateGuest = async (guest: Guest) => {
      try {
        if (selectedEvent) {
          ...
          await updateGuestAttributesBy(email, guest.id.toString());
        }
    ...
    return (
      <motion.div
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-4">
          ...
          {/* <div className="bg-white rounded-lg p-4 w-full md:w-auto flex space-x-2 items-center">
